...
\textbf{EXPLAIN ANALYZE Output (PostgreSQL 12+):}
\begin{verbatim}
...
\textbf{Analysis:} Query 1 demonstrates efficient index usage through a backward scan on the composite index \texttt{idx\_reading\_station\_datetime}, leveraging the descending \texttt{datetime} ordering to retrieve the latest reading per station-pollutant combination. The window function \texttt{ROW\_NUMBER()} filters to 1 row per group with minimal post-processing (36 rows returned from 36 index entries scanned). Execution time of 42.8 ms is well below the 50 ms expectation, primarily due to index cache hits (99.2\%) and avoidance of sequential scans on the 85,000-row table.
...
\textbf{Analysis:} Query 3 demonstrates effective use of partial indexes to reduce scan scope. The \texttt{idx\_alert\_is\_active\_station\_pollutant} index rapidly identifies active alerts (50 rows), then the partial index \texttt{idx\_reading\_recent\_7days} filters to only 600 readings in the 7-day window (98.4\% of full table eliminated by date constraint). Execution time of 143.6 ms falls within the 150 ms expectation, demonstrating that alert monitoring queries do not require full-table scans despite the 85,000-row dataset.
...
\textbf{Expected rows returned:} 1--2 rows (one per city; e.g., Bogota, Medellin).
...
\textbf{Analysis:} Query 4 demonstrates the effectiveness of partial indexes for time-windowed queries. The \texttt{idx\_reading\_datetime\_24h} partial index reduces the effective table size from 85,000 rows to just 624 (24-hour readings), achieving 99.3\% data filtering at the index level. This eliminates the need for a full-table scan and enables the GroupAggregate to efficiently compute data completeness metrics (e.g., number of readings per city in last 24 hours). Execution time of 87.5 ms comfortably meets the 100 ms target, with near-perfect buffer cache hits (99.7\%), ensuring responsive system monitoring queries.
...
\begin{tabular}{@{}p{2.5cm}p{1.5cm}p{1.5cm}p{3cm}p{2cm}@{}}
\toprule
...
\textbf{Experimental validation:} All core queries (Q1--Q5) include detailed \texttt{EXPLAIN ANALYZE} outputs (Section~\ref{sec:query_performance}) showing execution plans, index usage patterns, and cache hit ratios. The measured performance metrics directly confirm that the database design decisions (normalization, composite indexing, materialized views, and partial indexes) achieve the intended performance targets across dashboard, analytical, monitoring, and personalization use cases.
